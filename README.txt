...
4) Determine if your mac uses bash of zsh. Then, in the command line, write "touch ~/.[bash or zsh]rc", then "nano ~/.[bash or zsh]rc", then "export PATH="$PATH:[Path to flutter bin"], then control O, Enter, Control X, Y, and then save the line "source ~/.[bash or zsh]rc". Run "flutter doctor" to ensure proper setup.
